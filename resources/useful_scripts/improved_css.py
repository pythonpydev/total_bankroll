...
for rule in sheet:
    ...
    # Skip pseudo-elements explicitly
    if not selector_text or '::' in selector_text or selector_text.startswith(':'):
        pseudo_element_selectors.append(selector_text or '<empty selector>')
        continue

    try:
        # Validate selector syntax with expanded allowed characters
        ...
        for html_file in html_files:
            try:
                ...
                try:
                    ...
                    problematic_rules.append(f"Soup select error for '{selector_text}' in {html_file}: {e}")
                    continue
            except Exception as e:
                problematic_rules.append(f"Error reading HTML file {html_file}: {e}")
                continue
    ...
    if rule_count % batch_size == 0:
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write('\n'.join(pseudo_element_selectors + skipped_selectors) + '\n' if pseudo_element_selectors or skipped_selectors else "<none>\n")
            f.write("\nPseudo-element selectors:\n")
            f.write('\n'.join(pseudo_element_selectors) + '\n' if pseudo_element_selectors else "<none>\n")
            ...
            f.write(f"Total skipped selectors: {len(pseudo_element_selectors + skipped_selectors)}\n")
            f.write(f"Total pseudo-element selectors: {len(pseudo_element_selectors)}\n")
