### Steps to Purchase a Domain Like poker-profits.com and Link It to a PythonAnywhere Website

Purchasing a domain name and pointing it to a hosting service like PythonAnywhere is a straightforward process, but it involves two main phases: acquiring the domain from a registrar and configuring DNS settings to link it to your site. Note that PythonAnywhere's free accounts (like the one implied by your example URL, https://pythonpydev.pythonanywhere.com/) **do not support custom domains**. You must upgrade to a paid plan (starting at $5/month for the Hacker plan) to use a custom domain for hosting a website. Free accounts are limited to subdomains like `yourusername.pythonanywhere.com` or custom subdomains of PythonAnywhere's domain.

Assuming you have (or will upgrade to) a paid PythonAnywhere account and your Flask app (like Total Bankroll) is already set up and running on the free subdomain, here are the detailed steps. The process typically takes 1-2 hours to set up, plus up to 24 hours for DNS changes to propagate across the internet.

#### Phase 1: Purchasing the Domain (e.g., poker-profits.com)
Domain names are registered through ICANN-accredited registrars. Popular, reliable options include GoDaddy, Namecheap, Google Domains, or Hostinger. Costs for a .com domain like poker-profits.com are usually $10-15 for the first year (renewals may be $15-20/year), but check availability as prices can vary if it's premium or already taken.
...
1. **Choose and Search for the Domain**:
   ...
   - Use their domain search tool: Enter "poker-profits.com" (or variations like poker-profits.net if .com is unavailable).
...
2. **Select Registration Options**:
   - Choose the top-level domain (TLD): .com is standard and trusted for business sites like a poker bankroll app.
...
- If poker-profits.com is taken, search auctions on GoDaddy Auctions or Sedo for expired/premium domains.
...
Once purchased, configure your PythonAnywhere web app to use the custom domain and update DNS to point traffic to PythonAnywhere's servers. This uses a CNAME record (recommended for reliability). Your app (e.g., the Total Bankroll Flask site) must be configured for the new domain in your code (e.g., update ALLOWED_HOSTS in settings.py for Django/Flask).
...
2. **Configure the Web App for the Custom Domain**:
   ...
   - If creating a new app: Click "Add a new web app," select your domain (`www.poker-profits.com`), choose Python/Flask, and set up your code/files as before (upload your Total Bankroll app if needed).
   - Reload the web app (green "Reload" button). PythonAnywhere will now expect traffic for `www.poker-profits.com`.
   - Update your app code: In your Flask/Django settings, add the new domain to `ALLOWED_HOSTS` (e.g., `ALLOWED_HOSTS = ['www.poker-profits.com', 'poker-profits.com']`). Reload again.
...
3. **Set Up DNS Records at Your Registrar**:
   ...
   - For naked/apex domain (poker-profits.com without www): CNAMEs don't work here. Instead:
     - Use your registrar's forwarding/redirect tool to redirect `poker-profits.com` to `www.poker-profits.com` (e.g., GoDaddy's "Forwarding" feature).
...
4. **Test and Secure the Setup**:
   - Wait for propagation, then visit `www.poker-profits.com`—it should load your Total Bankroll app.
...
This setup will make your Total Bankroll app accessible at poker-profits.com, improving professionalism for tracking poker funds and assets. If the domain is unavailable, search for alternatives during purchase. For poker-themed domains, check for .poker TLDs (~$8-10 via Namecheap/GoDaddy) if .com is taken.
